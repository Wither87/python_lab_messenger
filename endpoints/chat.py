...
from schemas.chat import Chat, ChatInDB, LastMessages
...
@router.delete("/{chat_id}", response_model=ChatInDB)
...
@router.get("/last/{chat_id}", response_model=LastMessages)
async def get_last_messages_in_chat(chat_id: int, quantity: int = 10, user_id=Depends(get_current_user), db=Depends(get_db)):
    ...
    return LastMessages(chat=_chat, messages=_messages)


@router.get("/activity/{chat_id}", response_model=list[ChatInDB])
